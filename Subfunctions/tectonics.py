import numpy as np
from geometry import geographic_to_cartesian, cartesian_to_geographic, pitch_yaw_rotation_matrix
...
class Stationary_Grid:
    def __init__(self, grid, height, velocity, plate_type, resolution) -> None:
        self.grid = grid
        self.height = height
    ...
    def move_plates(self):
        ...
        for i in range(self.size):
            ...
            new_grid_point = cartesian_to_geographic(point)[0]
            new_index = int(new_grid_point[0]//(self.resolution_step[0]) + ((new_grid_point[1])//self.resolution_step[1])*self.resolution[0])
            next_step_height[new_index] += self.height[i]
            if next_plate_step[new_index] < self.plate_type[i]: #check if it is the dominant plate
                #transferring information
                next_step_velocity[new_index] = self.velocity[i] 
                next_plate_step[new_index] = self.plate_type[i] 
            elif self.plate_type[i] < 0:
                self.volcanism[new_index] += 0.1 # to allow for simulated volcanism creating mountains and island chains
    ...
    def display(self):
        ...
        xx, yy = np.meshgrid(np.linspace(0, 2, self.resolution[0]), np.linspace(0, 1, self.resolution[1]))
        height_map = self.height.reshape(self.resolution)
...
class Tectonics:
    ...
    def gen_vor_tes(self):
        ...
        self.grid_coords = np.array([xx.flatten(), yy.flatten()]).transpose()
        #convert this to a 3d grid of points
        grid_coord_3d = geographic_to_cartesian(self.grid_coords)
    ...
    def evolve(self):
        ...
        grid = Stationary_Grid(geographic_to_cartesian(self.grid_coords), height_map[self.grid_value], plate_velocity[self.grid_value], plate_information[self.grid_value],self.resolution)
        grid.display()

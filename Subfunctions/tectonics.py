...
class Stationary_Grid:
    def __init__(self, settings, grid, grid_tree, height, velocity, plate_type, resolution) -> None:
        ...
        self.volcanism_spots = np.random.random((int(self.settings['VOLCANIC_SPOTS']), 3))
    ...
    def move_plates(self):
        ...
        for i in range(self.size):
            ...
            if next_plate_step[new_index[0]] < self.plate_type[i]:
                ...
                next_step_height[i] = 0  # to try and simulate trenches
        # averaging the poles
    ...
    def weather(self, time_of_year):
        ...
        self.humidity += np.minimum(1, np.maximum(0, self.temperature/100)
                                    )*(self.height <= self.settings['SEA_LEVEL'])/10
        ...
        self.temperature -= self.settings['TEMPERATURE_RADIATE']
    ...
    def temperature_update(self, time_of_year):
        ...
        # np.arccos(np.dot((0, np.sin(Planet_angle), np.cos(Planet_angle)), (np.sin(year_progress), np.cos(year_progress), 0)))
        ...
        solar_energy = np.sin(np.minimum(np.maximum(
            self.grid_2d[:, 1] - angle_between_sun, 0), np.pi))*Solar_Energy*proportion_day/(4*np.pi*np.square(Distance))
    ...
    def erosion(self, time_of_year):
        ...
        for i in range(self.size):
            ...
            minimum_min = near_points[np.argmin(
                self.height[near_points] + self.water_level[near_points])]
    ...
    def eruptions(self):
        ...
        for index in indices:
            self.height[index] += np.random.random()/self.settings['VOLCANIC_FACTOR']
        self.height += self.volcanism * \
            np.random.random(self.size)*(self.volcanism >= 1)/self.settings['VOLCANIC_FACTOR']
    ...
    def evolve(self):
        ...
        for i in range(int(self.settings['WEATHER_STEPS'])):
            self.eruptions()
            self.erosion(i/self.settings['WEATHER_STEPS'])
...
class Tectonics:
    def __init__(self, point_num, settings=None):
        if settings:
            self.settings = settings
        else:
            self.settings = SETTINGS
    ...
    def evolve(self):
        ...
        velocity = np.random.random((self.points, 3))/self.settings['PLATE_VELOCITY_FACTOR']
        plate_velocity = []
        for velo in velocity:
            plate_velocity.append(Rotation.from_rotvec(velo))
        plate_velocity = np.array(plate_velocity)
        grid = Stationary_Grid(self.settings, self.grid_coord_3d, self.grid_tree,
                               height_map[self.grid_value], plate_velocity[self.grid_value], plate_information[self.grid_value], self.resolution)
    ...
    def time_step(self, plate_position, plate_velocity):
        ...
        plate_position[:, 0] = (plate_position[:, 0] +
                                np.pi*plate_position[:, 1]) % 2*np.pi
    ...
    def display(self):
        ...
        for i in range(self.points):
            ...
            for n in range(len(self.grid_coords)):
                if self.grid_value[n] == i:
                    test_values.append(self.grid_coords[n])
